...
**Naming**: `test_param{N}_defined_nan_not_denied` / `test_param{N}_undefined_{sentinel}_accepted` / `test_param{N}_undefined_nonsentinel_rejected`. Each test's docstring first line is the single source of truth for its pass case — reused verbatim in the results log and any README table. On `Tier1CommandTestBase` (below), checks 4/5 are pytest-parametrized instead (`test_undefined_param_sentinel_accepted[param5]`) — the bracketed id plays the same role.

**Results log**: every `test_command.py` run writes `logs/command_<command>_tier1_<autopilot>_<vehicle>_<version>_<timestamp>.log` (test name, outcome, `MAV_RESULT`, pass case), always, regardless of pass/fail. `_check()`/`_record()`/`_write_tier1_log` live in `conftest.py`, reusing `_format_autopilot_header()` from `tests/conftest.py`.
...
**Migration status**: `external_wind_estimate` and `nav_takeoff` are migrated. `nav_land`, `do_reposition`, `do_set_mission_current`, `do_set_global_origin` still hand-roll some or all of the six checks.
...
- **EXTERNAL_WIND_ESTIMATE (cmd=43004, development.xml)** — see `external_wind_estimate/README.md`. COMMAND_LONG primary. Two confirmed PX4 findings: (1) a Commander/EKF2 dual-ACK race — `EXTERNAL_WIND_ESTIMATE` is missing from Commander's "handled elsewhere" ignore-list, so it falls through to `UNSUPPORTED` racing EKF2's `ACCEPTED`; (2) a DOC DISCREPANCY — `resetWindToExternalObservation()` is gated `if (!in_air)`, so the estimate is silently dropped while airborne even though the spec explicitly describes an in-flight use case and the ACK is unconditionally `ACCEPTED` either way.

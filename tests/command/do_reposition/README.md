...
|------|--------|-----|-------|
| `test_denied_not_in_hold` | XFAIL | 0 | PX4 SIH auto-transitions to AUTO_LOITER after EKF convergence before the test runs, so param2=0 → ACCEPTED (branch 2), not DENIED (branch 3). Only verifiable on real hardware or with an explicit pre-test mode reset. |
| `test_accepted_change_mode` | PASS | 0 | CHANGE_MODE → ACCEPTED, switches to AUTO_LOITER |
| `test_accepted_already_in_hold` | PASS | 0 | param2=0 while in Hold → ACCEPTED (branch 2) |
| `test_command_accepted` | PASS | 0 | |
| `test_param2_change_mode_flag` | PASS | 0 | |
| `test_param2_flags_zero` | PASS | 0 | already in Hold from prior tests |
| `test_param2_relative_yaw_only` | PASS | 0 | already in Hold |
| `test_param2_all_flags` | PASS | 0 | |
| `test_param2_undefined_bits` | PASS | 0 | bit 0 (CHANGE_MODE) set in 255 |
...
| `test_param4_yaw_zero` | PASS | 0 | applied as heading setpoint |
| `test_param4_yaw_specific` | PASS | 0 | |
| `test_param4_relative_yaw_with_flag` | PASS | 0 | |
| `test_param3_zero` | PASS | 0 | |
| `test_param3_nan` | PASS | 0 | |
| `test_param3_positive` | PASS | 0 | MC accepts non-zero radius — spec gap, should DENY |
| `test_param3_negative` | PASS | 0 | negative radius accepted |
| `test_location_specific` | PASS | 0 | |
| `test_location_int32max` | PASS | 0 | |
| `test_location_out_of_range_latlon` | XFAIL | 0 | coordinate range not validated — spec gap |
| `test_altitude_nan` | PASS | 0 | |
| `test_altitude_zero` | PASS | 0 | |
| `test_altitude_only_reposition` | PASS | 0 | |
| `test_all_nan_pause` | PASS | 0 | all-NaN "pause" accepted via COMMAND_LONG |
| `test_command_long_nan_latlon` | PASS | 0 | |
| `test_command_long_int32max_float` | XFAIL | 2 | DENIED — PX4 treats float(INT32_MAX) as a protocol error rather than the valid "use current position" sentinel |
...
| Test | Result | ACK | Notes |
|------|--------|-----|-------|
| `test_denied_not_in_hold` | PASS | 2 | not in Hold → DENIED, matches PX4 mode-gating |
| `test_accepted_change_mode` | **FAIL** | 4 | CHANGE_MODE set; expected ACCEPTED, got FAILED — see finding 1 below |
| `test_accepted_already_in_hold` | SKIP | — | depends on a prior test reaching Hold mode |
| `test_command_accepted` | PASS | 4 | baseline — ArduCopter responds FAILED, not UNSUPPORTED |
| `test_param2_change_mode_flag` | **FAIL** | 4 | expected ACCEPTED, got FAILED |
| `test_param2_flags_zero` | PASS | 2 | no flags → DENIED |
| `test_param2_relative_yaw_only` | PASS | 2 | RELATIVE_YAW, no CHANGE_MODE → DENIED |
| `test_param2_all_flags` | **FAIL** | 4 | CHANGE_MODE\|RELATIVE_YAW(3); expected ACCEPTED, got FAILED |
| `test_param2_undefined_bits` | PASS | 4 | |
| `test_param1_default_speed` … `test_param4_relative_yaw_with_flag` | PASS | 4 | 8 param1/param4 value sweeps, no denial |
| `test_param3_zero` / `test_param3_nan` / `test_param3_positive` / `test_param3_negative` | PASS | 4 | |
| `test_location_specific` | PASS | 4 | |
| `test_location_int32max` | PASS | 2 | INT32_MAX ("use current position") → DENIED — spec violation, same family as ArduPilot mission-protocol NAV_TAKEOFF sentinel rejection |
| `test_location_out_of_range_latlon` | PASS | 2 | out-of-range lat/lon correctly DENIED |
| `test_altitude_nan` | PASS | 2 | NaN ("use current altitude") → DENIED — spec violation, should accept |
| `test_altitude_zero` / `test_altitude_only_reposition` / `test_all_nan_pause` / `test_command_long_nan_latlon` / `test_command_long_int32max_float` | PASS | — | UNKNOWN — no ACK within 5s (see finding 2 below) |
| `test_param1_below_min` | XFAIL | 4 | −5.0 (below minValue=−1) accepted instead of denied — spec gap, same as PX4 |
...
2. **ACK responsiveness degrades mid-sequence.** From `test_param1_nan_speed` onward roughly 14 probes in, the pattern trails off into no-ACK; no command-queue/dedup/throttle logic in `GCS_Common.cpp` would explain a permanent stop mid-session. Candidates: a downlink backlog from repeated `LOGGER_WRITE_ERROR`/STATUSTEXT emissions (one per failed `set_destination()`) starving `COMMAND_ACK`, or a test-harness-side `mavlink_direct`/gRPC artifact (see `[[grpc_cancel_pattern]]`). Needs runtime packet capture (`mavlogdump`/Wireshark) to confirm, not further source review.

### ArduRover — 2026-06-08

27 passed, 3 failed, 1 skipped. Log: `logs/command_do_reposition_ardurover_20260608.log`.

Same outward pattern as ArduCopter (mode-gated DENIED when correctly gated, FAILED on the three CHANGE_MODE-asserting tests, ACKs stop entirely partway through — after ~12 probes here, from `test_param1_nan_speed` onward, vs. ~14 on ArduCopter but at the identical trigger test) — full per-test table: `logs/command_do_reposition_ardurover_20260608.log`.
...
2. ACK loss starts at the same test (`test_param1_nan_speed`) on both vehicles despite different firmware — argues against coincidence. No ACK-suppression mechanism found in `GCS_Common.cpp` for either vehicle. Two live candidates: the GUIDED-mode transition itself (plus associated failsafe/STATUSTEXT traffic), or something in the `param1=NaN` encoding common to both vehicles. Needs runtime packet capture to distinguish, not further source review.
...
| Test | Result | Notes |
|------|--------|-------|
| `test_denied_not_in_hold` / `test_accepted_change_mode` / `test_accepted_already_in_hold` | SKIP | mock has no mode state |
| `test_param1_nan_speed` / `test_param4_relative_yaw_with_flag` / `test_param3_nan` / `test_altitude_nan` / `test_all_nan_pause` / `test_command_long_nan_latlon` / `test_command_long_int32max_float` | SKIP | requires real stack |
| `test_param1_below_min` | XFAIL | mock accepts −5 m/s (below minValue=−1) |
| `test_location_out_of_range_latlon` | PASS | mock correctly DENIEDs out-of-range coordinates |
